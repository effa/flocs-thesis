...
and insted restrict our domain to non-overlapping problem sets, which are
easier to handle.
% ... We started by manually dividing tasks into 9 problem sets (levels)
We have devided tasks into 9 ordered disjoint problem sets
(\emph{missions}, \emph{levels}),
mostly based on the available programming blocks and contained game elements,
and then we have further refine each mission into approximately 3 orderd
disjoint groups (\emph{phases}), in order to enforce the most important
prerequisities within each mission (e.g. introducing worm holes before
using them in more difficult tasks).
...
and tasks inside the same phase should be approximately homegeneous.
TODO: tasks within phase -- unordered, interchangeable, about 5?
...
(typically 3 phases corresponding to introduction, recombination, reinforcement/practice)
...
\centering
TODO: domain diagram
(all 9 missions; (mostly/all) unlabeled nodes for phases and tasks)
