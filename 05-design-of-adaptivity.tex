...
The tutor is hierachical, first selecting a problem set, then a task from
this problem set. In order to asses if a problem set is mastered,
...
in which case it is preferable to solve them in order from the
easiest to the most difficult.
Homogeneous phases allow to select a task uniformly at random from
all unsolved tasks in the phase, which is a strategy maximizing exploration.
% UI:
Recommendations provided by the system are \emph{soft};
i.e. that student can ignore the recommendation and select any task
...
Perceived difficulty increases after progess to a new mission or phase,
but it decreases during solving tasks from a single phase,
as the student improves her skills, while the objective difficulty stays
approximately same (\cref{fig:robomission.flow}).
On one hand, it means that the difficulty of the tasks is not always optimal
for the student, possibly slightly overshooting at the beginning of the phases,
...
for students However, the resulting \emph{dramaturgy of flow}
is more interesting for students \cite{book-of-lenses}.
