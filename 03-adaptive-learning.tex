...
\item ?? input/output (TODO: consider moving here the performance
domains/represenations discussed in next section)
...
The more complex model, the more precisely it can capture the reality, but only
...
\item ?? consider using $[0, 1]$ range to be in line with the multivalued logics
  ...
  table) \item Example of performance compression/extraction:
...
\item While the compression causes a loss of information,
  ...
  easier to interpret.
\item Alternative performance values domains/representations:
  \begin{itemize}
  ...
  \item continuous value combining time and correctness -- e.g. math problems,
      where correctness is the main criterion, but fluency is also important
      (REF matmat)
  \end{itemize}
