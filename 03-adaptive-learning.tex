...
Entities in the domain (called \emph{chunks}) include tasks
...
In addition to the data, domain models include algorithms. Domain algorithms can be
...
and collected historical performance data into a compact domain representation.
Typically, performance data are used only to learn parameters
...
that are required for solving the task.
...
concepts are called either \emph{disjoint} (1:m) or \emph{overlapping} (n:m)
(\cref{fig:concepts-disjoint-overlapping}).
...
Overlapping concepts can be represented as sets of tasks, bipartitie graph,
or a feature matrix (\emph{Q-matrix}), where rows correspond to tasks, columns to concepts,
and values denote whether given task contains given concept (1), or not (0)
(\cref{fig:qmatrix}).
...
Manually selected concepts are interpretable and do not require collected data,
...
Hierachy can be modeled either as a rooted tree (single parent concept allowed),
...
a more general loops concept), \cite{learner-models-integration-skills} shows
that explicitly modeling the hierarchy of concepts in the introductory
programming can improve accuracy of the overlaying student model.
...
Another relationship between chunks are prerequisites. They specify chunks
that have to be mastered prior to tackling given chunk.  % TODO: grammar?
For example, single loop is a prerequisity for nested loops.
If chunks has multiple prerequisities, there are two common interpretations:
all of the \emph{conjunctive prerequisities} must be mastered,
while only a single from the set \emph{disjunctive prerequisities} is enough.
We can describe both of these types in an \emph{and-or graph} (\cref{fig:chunks-prerequisites}).
However, if only conjunctive prerequisites are needed, then it is enough to represent
prerequisities by a DAG, called \emph{Partial Order Knowledge Structure} (POKS) (CITE).
More genererally, each chunk can specify a function from skills of its prerequisities
into a prior skill (and leave the \emph{ready-to-tackle decision} on a tutor model).
If the skills can be interpreted as probabilities, then the resulting prerequisity structure
...
Similarity is a symetric relationship between chunks of the same type, for example tasks.
It can be represented either as a matrix of soft similarities, or as a graph with
edges between the most similar chunks (\cref{fig:similarities-tasks}).
...
and it depends both on the intended usage and on the amount of collected data
available (e.g. performance data are useful if the similarities are used in the student
model to predict future performance; however, it requires that enough performance
data is collected so that the computed similarities are stable).
...
\begin{subfigure}{.48\textwidth}
\centering
\includegraphics[width=0.78\textwidth]{img/similarities-tasks}
\end{subfigure}
\begin{subfigure}{.48\textwidth}
\centering
\includegraphics[width=0.78\textwidth]{img/similarities-graph}
\end{subfigure}
\caption{%
  Left: Similarity matrix between tasks;
...
or each task as separete concept. %(REF: slepe-mapy).
...
(CITE), % TODO:CITE: rule-based models, constraint based models),
while others only describe the state of the student between task sessions
\cite{kli-framework}. % TODO: check
...
Student model use a domain model as an underlaying structure,
...
\label{sec:elo}  % TODO: rename label

Logistic models \cite{irt-visual-guide}  % TODO:check-cite
...
Elo model \cite{alg.elo, irt-elo-math} extends the logistic model
...
underlaying domain (inclusion, prerequisities, and generalization).  % TODO:check.
...
depends on to the probability distribution of its values.  % TODO: check
...
for fast update of hidden skills. % and prediction of performances.
...
such as \emph{predictive accuracy}, % \cite{pelanek-evaluation-student-models} %(TODO:+page),
\emph{reliability and resolution} %of predictions
\cite{pelanek-evaluation-student-models}, %(TODO:+page),
...
and to use several proxy quality measures.  % or "evaluation approaches"
...
\emph{Tutor models} (a.k.a. \emph{instructional policy}) (CITE) %, \emph{instructional strategy})
...
allows to learn action preferences using future rewards derived from the estimated time
spent in flow \cite{rl-for-tutors-evaluation}.  % TODO:FIX: now it looks that the referenced article uses flow
% TODO: Explore->cite.
...
In mastery learning (CITE+\cite{mastery-learning-scale}), each topic
...
model to a threshold. % "calibrated threshold"
...
single-difficulty descriptive student models,  % TODO: make sure the terms are defined
...
Both for the prerequisities and follow relation, it is important that the domain model
maches reality. To some extent, offline analysis can be used to either verify, improve or even
...
and following prerequisities.  % prerequisities are more suited for PS
...
To balance exploration and epxloitation, the system can either select tasks at random
...
%Supervise learning
Evaluation of accuracy of recommendations requires to define which
...
adaptive behavior of the system, or by the self-selection bias. % TODO:CITE
...
Monthly Active Users & students who solved $\geq$ 10 tasks in last 30 days \\
...
These metrics are called \emph{online attributable metrics}.  % TODO: find used terminology
% OR "short term metrics"
%and this type of evaluation \emph{live evaluation}.
...
Offline experiments allows to reuse the same data to evaluate several models,
  obtaining the results quickly,
  and avoiding a potential negative impact on students if the evaluated model is poor.

The specific approaches and challenges of evalution of student and tutor models
...
For an unbiased offline evaluation, it is important to test the model on different data
then it was trained on. Unlike in many supervised learning problems, events from ALS
