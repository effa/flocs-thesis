...
In principle, either the weights of individual error functions, or even the error functions
themselves can be chosen to optimize a desired metric, but there are two complications.
First, the training data are sparse, because only a single followed recommendation was
observed for each student in given state. % TODO: formulate better
Second, it is not clear whether the recommendation was correct % TODO: elaborate
and proxies must be used (e.g., recommendation is declared as good if the
observed performance was neither too low, nor too high).

To balance epxloration and epxloitation, the system can either select tasks at random
