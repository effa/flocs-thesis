...
\item What it is:
  ...
  used to predict her future performance

\item Usage:
  \begin{itemize}
  \item In tutor models: detecting mastery (skill is high enough),
    task/PS recommendation (predicted performance is neither too low, nor too high,
    suggesting that the task is of optimal difficulty).
...
\item comprehensive overviews of student modeling in general:
  \cite{its-learner-models, student-models-review-2012, pelanek-learner-modeling}
  (in this section we describe only the ones most relevant for the domain
    of introductory programming)

\item We restrict our focus to modeling skills of the student.
  However, broade student models may also include affect (e.g. frustration,
...
\item We first compress these data into a discrete
  performance measure $p_{ij} \in \{-1, 0, 1\}$, where
...
\item There is a loss of information during the compression,
  but it helps to avoid combinatoric explosion (considering each type of model
  with multiple different types of input data or their combination).
  Having a unified simple performance input makes the models simpler and
  easier to interpret.
\item ?? Furthermore, there is a lot of noise in the raw data, which is largely
  reduced by the compression step.
\item Alternative performance valued domains:
  \begin{itemize}
...
\begin{itemize}
\item (?) definition:
  $\theta_{cj} \in [-1, 1]$ expresses how much student $j$ knows concept $c$
  (-1 not at all, 1 fully known, 0 ? initial student / ready to learn)
