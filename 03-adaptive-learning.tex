...
for a simple online update: % of hidden skills: % and prediction of performances.
...
% TODO: Fix: assymetric text - should include a section on evaluation of domain models
...
However, evaluating the impact of the student model is difficult, % TODO:CITE:example
...
% ... which are currently better performed by people than by computers \cite{als-future}.
% ... "so adaptive learning systems often focus only on the outer loop decisions"
% Of course, human instructors are not always available, so it make sense
% to develop an inner loop tutor as well.
...
% computers in inner loop (or both) (in the cited paper, they state the 2nd one)
...
Unfortunately, collected data are highly biased by the
...
%% EXT (removed to make the thesis shorter; but it is highly relevant)
...
The user interface (UI) is also a common subject of personalization. % (TODO:CITE).
...
task stories can use student's favorite characters. % and colors.
...
To decide which interventions improve learning and motivation, % the quality of the system,
...
was observed to hold for adaptive learning systems \cite{stupid-tutoring-systems-intelligent-humans,its-domain-models}, % TODO: better resource?
%\cite[p.124]{its-domain-models, stupid-tutoring-systems-intelligent-humans}
...
(e.g., after conducting an AB experiment). % TODO: cite A/B tests, in the context of ALS or RecSys
...
Limitation of these simple metrics is that they only capture \emph{engagement},
not learning.
