...
\footnote{They are more often called \emph{Intelligent Tutoring Systems} \cite{its-overview}. We prefer to use the term Adaptive Learning System to emphasize learning (goal), rather than teaching (means).}
...
personalize the sequence of tasks (CITE), %  for every student,
...
tasks belong to a single undivisible concept.
...
(\cref{fig:concepts-tasks-relationships} a, b).
...
or a feature matrix (\emph{Q-matrix}), where rows correspond to tasks, columns
to concepts,
...
\imgW[0.9]{concepts-tasks-relationships}{%
  %Comparison of disjoint and overlapping concepts.
  In the disjoint model (a), each task (light squares) belong to one concept
  (dark rectangles).
  In the overlapping model (b), each task can contain multiple concepts.
  Membership in concepts can be soft, described by a continuous value
  (shade of gray in (c)).}
...
\imgW{concepts-hierarchical}{Hierarchial concepts as a tree (left), or a DAG (right).}
...
Prerequisities specify which learning objects need to mastered prior to
tackling a given learning object.
For example, single loop is a prerequisity for nested loops.
If learning objects has multiple prerequisities, there are two common interpretations:
...
Both conjuctive and disjunctive prerequisites can be represented by a single
\emph{and-or graph} (\cref{fig:chunks-prerequisites}).
If only conjunctive prerequisites are needed, then it is enough to represent
prerequisities by a DAG, called \emph{Partial Order Knowledge Structure}
(POKS) (CITE).
More genererally, each learning object can specify a function from skills of
its prerequisities
