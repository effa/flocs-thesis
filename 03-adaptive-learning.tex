...
\item Usage:
  ...
  \item In UI: showing feedback for the student
        (visualization of her skills -- "open learner model").
...
\item Comprehensive overviews of student modeling:
  ...
  In this section, we describe only the models most relevant for the domain
  of introductory programming.
  We further restrict our focus to modeling skills of the student.
  ...
  We also restrict our attention to the models that describes a state of student
  between two task sessions (REF: KLI framework?), as these can be used for task
  reccomendation and mastery detection.
  Other models exist that describes state of the student solving a particular
  task (REF: rule-based models, constraint based models).
\end{itemize}
...
Algorihtms:
\begin{itemize}
\item \emph{offline learning} to find the population parameters,
\item \emph{online update} of student skills based on an observed task session,
\item \emph{prediction} of performance for given student-task.
  (?? also prediction of skill for student-concept,
  if the data are more complex, e.g. hierarchical model?),
\item TODO: \emph{insisht} (visualization learning curves, ...).
