...
\begin{itemize}
\item TODO: define
\item aka instructional policy/strategy
\item Includes several tasks, common classification:
  within a single task (\emph{inner loop}),
  between task sessions (\emph{outer loop}),
  but both can be further decomposed hierarchically
...
\item What (ordered from the most outer to the most inner loop):
\begin{itemize}
\item \emph{Curriculum sequencing} (Content sequencing?)
  What topic to learn next? (What problem set to practice next?)
  Relevant for systems for a lot of diverse content with prerequisity structure.
...
\item \emph{Adaptive scaffolding} (??) --
  ...
  (instruction, hint, worked out example, explatanation)?
  (or even a suggestion to give up and try an easier task).
\end{itemize}
\item Often, tutor models use a domain and a student model as an input,
  but not always
  (model-free approaches exists, e.g. a classifier for task recommendation that
  uses the complete student history, or mastery learning with
  "3-correct-in-a-row" ...  though both of these cases can be viewed just as
  trivial special cases of student models) (REF).
\end{itemize}


Planning vs. single-action:
Currently, focus on single decision based on currently learned models
...
\emph{learning path}) towards the requested topic \cite{its-programming}.
