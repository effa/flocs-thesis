...
\begin{itemize}
\item TODO: definition of domain model: (?) model of tasks and relationships between them
\item TODO: what is domain model for? (1) used in student models,
  (2) visualization in UI, e.g. grouping tasks into problem sets?)
...
\item TODO: DAG of concepts -> Bayesian network \cite{its-programming}; another example (for math): Dybuster Calcularis (Design and evaluation of the computer-based training program calcularis for enhancing numerical cognition.)
...
prediction fn (student, task, domain, context, hyperparameters?) -> performance,
