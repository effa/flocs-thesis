...
include emotions (e.g. frustration, boredom), needs, motivation, and metacognition (CITE).
...
User interface is also a common subject of personalization (CITE).
% Not only in ALS, generally in HCI research.
% NOTE: also called "tutor-student interface"
User interface provides visualizations that are based on
the domain model (e.g. overview of taks and problem sets),
...
- interaction with what is good underlyaing model (e.g. task recommendation
different if the recommendation is soft/hard, if it shows one/two rcm...)

Reason to model:
- to perform AB experiments with several versions
- to personalize for a given student (but all the individual preferences should
  be already in the student model), UI decides how to visualize various elements
  based on the student models
