...
\item Usage:
  ...
  (TODO: example).
  \end{itemize}
\item No single best student model -- depends on its usage, and also on the
  type and amount of data we have.
\item Comprehensive overviews of student modeling:
  \cite{its-learner-models, student-models-review-2012, pelanek-learner-modeling}
  In this section, we restrcit our attention to the models relevant for the
  domain of introductory programming, that have the following properties:
  \begin{itemize}
  \item They model only skills of the student.
  Broader student models may also include affect (e.g. frustration,
  boredom), motivation, and metacognition (knowledge about their knowledge) (REF).
  \item They can model multiple skills, specifically with a conjunctive concepts
  as the underalying domain model. (REF section).
  \item They model the state of the student between two task sessions
  (REF: KLI framework?), as these can be used for task reccomendation and
  mastery detection.  Other models exist that describes state of the student
  solving a particular task (REF: rule-based models, constraint based models),
  \item They are \emph{online}, i.e. parameters can be updated gradually after
  each task session).
  \end{itemize}
\end{itemize}
...
\item Observed data from a task session:
  ...
  or even the individual edits.
\item We first compress these \emph{observational data} into a discrete
  \emph{performance measure} $p_{ij} \in \{-1, 0, 1\}$, where
  -1 = poor performance, 0 = good performance, 1 = excellent performance
  (corresponding to the too difficult / just right / too easy "flow levels").
\item ?? consider using $[0, 1]$ range to be in line with the multivalued logics
  (performance as a measure of success) (disadvantage: possible confusion with
  the probabilistic interpreation, which doesn't make much sense -- unless we
  assume a very specific shape of the probablistic distribution, we need to
  specify 2 numbers to get the probablity table)
...
\item While the compression causes a loss of information,
  ...
  easier to interpret.
\item Alternative performance values domains:
  \begin{itemize}
  \item binary correctness -- e.g. for multiple-choice questions (REF example),
  \item log-time -- e.g. for logic puzzles;
        most people can solve most puzzles given enough time, so the time
        is what distingueish the skill (REF proso),
  \item continuous value combining time and correctnes -- e.g. math problems,
      correctness is the main criterion, but fluency is also important (REF matmat)
  \end{itemize}
\item In introductory programming: most students solve most tasks given enough
  time, so correntess is out of question. We could still map task session data
  into a binary performance (1 = good, 0 = poor), but that would not allow
  to distingues between students for whom are the tasks just right and those
  for whom are too easy. Therefore we have chosen the three-valued domain,
  matching the predictions that we care about: whether a given task is for
  given student too difficult, just right, or too easy.
...
\item Each student model thus depends ("overlays") a domain model, which
  ...
  and describes how these concepts interact among themselves and how they
  relate to each task.
\end{itemize}
