...
% Ultimate objective decomposition/proxying:
% - need to start somewhere (position/paradigm): the higher level, the more difficult the derivation of metrics we will be actually able to use will be, but also it's important to make sure that we are doing a good thing
% But: "there is often no "true" objective" -- Google Rules of ML, 39
% Ultimate objective: "make the world a better place (as much as possible?)" (TBA: rephrase)
% -> (...) long-term increase in algorithmic-problem-solving skill summed over all people
% -> (...) having many satisied students who mastered elementary programming quickly while having fun (integrated over long-period of time, possibly infinite sum with exponential discount as in RL)
% -> "sum exponentialy-time-decayed flow-minutes over all users" [this is already long-term measurable, but requires asking on flow / or furhter proxying]

% Ultimate objectives are important, but impossible (or too difficult) to measure
% -> that's why we use proxy metrics:
% example: if the student is in flow, they will learn; flow is still difficult to measure,
