...
\item \emph{Offline analysis/evaluation/training} (Human in the loop).
\end{itemize}
...
TODO: \cite{pelanek-learner-modeling}: data + procedures
- data: KC structure + item parameters
- offline procedures: KC structure search -> structure, parameter fitting -> item parameters
- online procedures: (update equation - not necessarily online), prediction equation

Structure possibilities \cite{student-models-review-2012}:
- disjoint chunks (1:n chunks:tasks)
- overlapping chunks (m:n chunks:tasks) (Q-matrix)
- relationships between chunks:
  - hieararchy (generalization/specialization (or inclusion) relationship)
...
\item "knowledge space theory" (REF: Spaces for the assessment of knowledge.)
  - prerequisites between tasks -> DAG of tasks (KSI) (nodes = tasks, edges = prerequisities),
    can be extended to concepts -> DAG of concepts -> Bayesian network \cite{its-programming}; another example (for math): Dybuster Calcularis (Design and evaluation of the computer-based training program calcularis for enhancing numerical cognition.)
    - generally: AND/OR graph, special case is when all nodes are AND
    (than it is called "Partial Order Knowledge Structure" model)
...
\item skill-integration nodes (BN with 4 types of nodes: observations, base skills, integration skills and cognitive load node) \cite{learner-models-integration-skills}
\end{itemize}

NOTE: not only base skills, but also integration skills are important:
\cite{learner-models-integration-skills}

NOTE: having multiple-skills per item -> "credit assignement problem"
(which concept caused the poor performance)
(detail in \cite{pelanek-learner-modeling})
  - compensatory (additive) model
  - conjunctive (product) model
  - log-reg
  - weakest skill
  - DINA etc.
-> what makes most sense in our domain?
