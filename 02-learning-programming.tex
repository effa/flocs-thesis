...
Tasks & robot on a grid, drawing with turtle \\
...
Having five different basic actions is useful for a diversity of simple tasks.
...
like a puzzle, and in the second level, they learn loops and conditions by solving
...
motivation; however, the evidence is not sufficiently convincing. For example, in
...
To make the transition easier, the block representation of the code
  ...
  to which the students are expected to move in the next phase
...
In the last level of Blockly Games, which employ this strategy,
  the text on the blocks matches the generated JavaScript precisely
  \cite{blockly-10-things}.
...
For example, in turtle graphics,
  the whole state is just a position and orientation of the turtle,
  and effects are the drawn lines (\cref{fig:turtle}).
...
\Cref{fig:blockly-games} shows a possible solution to this problem:
  actionable non-ignorable instructions,
  which cannot be closed manually by the student,
  and disappear only once the student performs the action described in the instruction.

In addition to the instructions, some systems offer hints, which appear either
upon a student request or automatically after a certain time of unsuccessful solving.
Although it is possible to generate a hint in any state, using data of students
who have successfully solved the task in the past \cite{generating-hints},
most existing systems use a few manually prepared hints, instead of relying on
the automatic approaches.
...
\Cref{tbl:motivation-strategies} links the needs to common strategies.
...
even without tasks to solve,
  it should be an attractive toy to play with \cite{book-of-lenses}.
...
However, it is important for the students to feel ownership over the code,
  which is particularly a concern with the code skeleton.
...
and recommending tasks of the optimal difficulty.
...
solved tasks and completed problems sets,
acquired skills,
and progress towards mastery in the current topic.
