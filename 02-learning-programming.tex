...
Robotanist (figure \ref{fig:robotanist}) is similar to LightBot
...
% (figure \ref{fig:robomission-task2}).
Blockly Games%
...
demonstrate Blockly usage.
The webpage consists of several simple games, ordered by increasing difficulty.
...
Each game consists of 5-10 tasks, again ordered by increasing difficulty. %, with no personalization.
The fixed order enables to build on the program from the previous task,
thus gradually create more and more complex programs,
...
These tutorials focus on motivation, using videos where famous people explains
needed concepts, and themes from popular movies and games.  Tutorials use
high-level theme-specific blocks, such as ``set droid to a [random] speed``
and are restricted to only one or two programming concepts,
e.g. sequences and events.
...
in which the explained code can be fiddled with at any moment to understand
how it works.
...
A common strategy in the current systems for learning programming for avoiding
syntax errors is to replace textual programming with drag-and-drop block-based
programming.
There are two basic types of block-based interfaces:
  either a square grid defining the program shape,
