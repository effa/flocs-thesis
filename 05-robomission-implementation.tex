...
and well-readable without its previouws knowledge,
conciseness (short, but readable programs),
matching Blockly blocks as closely as possible
(for easy transiton from Blockly),
matching Python as closely as possible
(for easy transition to Python).

\subsection{RoboCode Grammar}

% TODO[consider] note on mixing lexer+parser+some semantic analysis
% (which is partially convenient, partially confusing)

There are for commands for performing actions:
\begin{lstlisting}
fly()
left()
right()
...
which was simplified to be easier to the form matching a corresponding
Blockly block, which is easier to comprehend by beginners:
...
Tests insided while-loops and if-statements are limited to the following form
...
for example ``r'' is red, etc.


\subsection{RoboAST}

\begin{itemize}
\item note about PEG grammar (+ issued with indentation)
...
\item TODO: explain/ref PEG grammar
\end{itemize}
