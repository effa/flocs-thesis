...
and a few specific aspects, such as representation of tasks,
...
% \section{Frontend}
Frontend is a single page application with \emph{redux architecture}
...
and defining actions to dispatch \cite{react}. %  and their triggers.
The application also defines a few asynchronous workflows (\emph{sagas}),
e.g. for program interpretation, or task solving process. %, or sending events to Google Analytics.  % better examples?
...
\emph{ViewSets} describing REST API, % TODO: cite view-sets
...
Domain is represented by a JSON file containing all problem sets %missions, phases, and
...
\emph{RoboCode} is Python-like programming language
...
\Cref{tbl:code-representation} shows overview of all representations.
...
The main advantage of PEG is that they do not require any lexer % TODO: check term
and the resulting grammar is still concise; %readable and short;
...
MiniRoboCode is useful not only for logging and storing programs in database,
...
%(See figure \ref{fig:roboblocks-english-czech} for an example of the same program in two languages.)
...
and then use a JavaScript interpeter %(CITE)
to run the code.
